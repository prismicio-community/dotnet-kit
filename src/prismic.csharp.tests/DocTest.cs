...
namespace prismic.csharp.tests
{
	...
	public class DocTest
	{
		...
		public void PrivateApiTest ()
		{
			...
			Api.Api api = prismic.extensions.Api.Get("MC5-XXXXXXX-vRfvv70", "https://lesbonneschoses.prismic.io/api").Result;
			// endgist
		}
		...
		public void ReferencesTest ()
		{
			...
			Api.Api api = prismic.extensions.Api.Get(previewToken, "https://lesbonneschoses.prismic.io/api").Result;
			var stPatrickRef = api.Refs["St-Patrick specials"];
		...
		public void GetTextTest ()
		{
			...
			var maybeAuthor = doc.GetText("blog-post.author");
			var author = "Anonymous";
			if (maybeAuthor.Exists()) author = maybeAuthor.Value();
			// endgist
		...
		public void GetNumberTest()
		{
			...
			var price = doc.GetNumber("product.price").Value().Item;
			// endgist
		...
		public void DateTimestampTest()
		{
			...
			var date = doc.GetDate("blog-post.date").Value().Item;
			var dateYear = date.Year;
			var updateTime = doc.GetTimestamp("blog-post.update");
			if (updateTime.Exists()) {
				var updateHour = updateTime.Value().Item.Hour;
			}
			// endgist
		...
		public void GroupTest()
		{
			...
			var group = document.GetGroup("article.documents").Value();
			foreach (var doc in group.Item) {
				// Desc and Link are Fragments, their type depending on what's declared in the Document Mask
				try {
					var desc = doc.fragments["desc"];
					var link = doc.fragments["linktodoc"];
				} catch (Exception e) {
					// Missing key
				}
			}
		...
		public void LinkTest()
		{
			...
			var resolver =
				prismic.extensions.DocumentLinkResolver.For (l => String.Format ("http://localhost/{0}/{1}", l.id, l.slug));
		...
		{
			var api = prismic.extensions.Api.Get("https://lesbonneschoses.prismic.io/api").Result;
			var response = api
				.Forms["everything"]
				.Ref(api.Master)
		...
		public void CacheTest()
		{
			...
			var cache = prismic.extensions.Cache.For<Api.Response> (
				(key, value, ttl) => {
					return null;
				},
				key => {
